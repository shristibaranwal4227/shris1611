...
func GetAllTasks(db *gorm.DB, w http.ResponseWriter, r *http.Request) {
	...
	meetingTitle := vars["title"]
	meeting := getMeetingOr404(db, meetingTitle, w, r)
	if meeting == nil {
		return
	}

	tasks := []model.Task{}
	if err := db.Model(&meeting).Related(&tasks).Error; err != nil {
...
func CreateTask(db *gorm.DB, w http.ResponseWriter, r *http.Request) {
	...
	meetingTitle := vars["title"]
	meeting := getProjectOr404(db, meetingTitle, w, r)
	if meeting == nil {
		return
	}

	task := model.Task{MeetingID: meeting.ID}
